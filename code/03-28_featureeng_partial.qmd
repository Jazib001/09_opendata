...
```{r}
fieldweather = read_csv("../data/Allsitesweatherdata.csv")
...
fe_month <- fieldweather %>%
  ...
  mutate(month.abb = month(date, label = T))
...
fe_month_sum = fe_month %>%
  group_by(year, site, month,
           month.abb, strength_gtex) %>%
  
  #for taking average
  
  summarise(across(c(dayl.s,
                     srad.wm2,
                     tmax.c,
                     tmin.c,
                     vp.pa
...
fe_month_sum_wide = fe_month_sum %>%
  ...
  mutate(varname = paste0(name, 
                          "_",
                          month.abb
                          )) %>%
  dplyr::select(-name, -month.abb, -month) %>%
